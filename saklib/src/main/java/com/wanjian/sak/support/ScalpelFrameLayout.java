...
//        return enabled || super.dispatchTouchEvent(event);
//    }
...
//    }
    ...
    public void onTouched(@SuppressWarnings("NullableProblems") MotionEvent event) {
        if (!enabled) {
            return;
        }
    ...
    public void draw(@SuppressWarnings("NullableProblems") Canvas canvas) {
        ...
        for (int i = 0, count = rootView.getChildCount(); i < count; i++) {
            ...
            if (child instanceof RootContainerView) {
                continue;
            }
            LayeredView layeredView = layeredViewPool.obtain();
            layeredView.set(child, 0);
